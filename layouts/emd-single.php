...
$container = apply_filters('emd_change_container','emd-wrapper','empd_com', 'single'); 
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'empd_com', 'single');
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
		...
		?>
<?php 	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'empd_com', 'single');
	if($has_navigation){
		global $wp_query;
...
<?php 	}
	...
	$show_edit_link = apply_filters('emd_show_single_edit_link',true,'empd_com');
	if($show_edit_link){
