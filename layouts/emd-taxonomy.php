...
$container = apply_filters('emd_change_container','emd-wrapper','empd_com', 'taxonomy');  
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'empd_com', 'taxonomy');
...
<?php 
	if($has_sidebar ==  'left'){
		do_action( 'emd_sidebar', 'empd_com' );
	}
	if($has_sidebar == 'full'){
?>
...
<?php
	do_action('emd_tax_before_header', 'empd-com',$post->post_type,$queried_object->taxonomy);
	emd_get_template_part('empd-com', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type) . '-theader');
	do_action('emd_tax_after_header', 'empd-com',$post->post_type,$queried_object->taxonomy);
?>
	</div>
...
<?php
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('empd-com', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type)); ?>
			</div>
...
<?php
	do_action('emd_tax_before_footer', 'empd-com',$post->post_type,$queried_object->taxonomy);
	emd_get_template_part('empd-com', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type) . '-tfooter');
	do_action('emd_tax_after_footer', 'empd-com',$post->post_type,$queried_object->taxonomy);
?>
</div>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'empd_com', 'taxonomy');
	if($has_navigation){	
		global $wp_query;
